...
# 创建选项卡
# 修改选项卡定义，添加新的标签页
# 原来的代码: tab1, tab2 = st.tabs(["合成字幕到视频 🔧", "从视频提取软字幕 📥"])
tab1, tab2, tab3 = st.tabs(["合成字幕到视频 🔧", "从视频提取软字幕 📥", "从视频删除字幕轨道 🗑️"])
...
with tab3:
    ...
    with col2:
        # 选择字幕轨道
        subtitle_track_remove = st.number_input("字幕轨道编号", min_value=0, value=0, step=1, key="subtitle_track_remove")

    # 使用列来组织输出格式和自定义文件名
    col3, col4 = st.columns(2)
    with col3:
        # 选择输出封装格式
        output_formats_remove = ["mp4", "mkv", "mov", "avi"]
        output_format_remove = st.selectbox("输出封装格式", output_formats_remove, key="output_format_remove")
    with col4:
        # 自定义输出文件名
        custom_output_filename_remove = st.text_input("自定义文件名（不包括后缀）", "", key="remove_subtitle_filename")
    
    # 生成命令按钮
    st.markdown("---")
    if st.button("生成删除字幕命令 🎛️", type="primary", key="generate_remove_command"):
        # 构造输出文件名
        if custom_output_filename_remove:
            output_filename = f"{custom_output_filename_remove}.{output_format_remove}"
        else:
            output_filename = f"no_subtitles_{video_file_remove.rsplit('.', 1)[0]}.{output_format_remove}"
        
        # 构建FFmpeg命令
        command = f"ffmpeg -i \"{video_file_remove}\" -c copy -map 0 -map -0:s:{subtitle_track_remove} \"{output_filename}\""
